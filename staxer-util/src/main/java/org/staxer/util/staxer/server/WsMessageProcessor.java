...
import org.staxer.util.staxer.StaxerException;
import org.staxer.util.staxer.domain.WssNonce;
import org.staxer.util.staxer.domain.WssPassword;
import org.staxer.util.staxer.domain.WssSecurity;
import org.staxer.util.staxer.domain.WssUsernameToken;
import org.staxer.util.LogUtils;
import org.staxer.util.ResourceUtils;
import org.staxer.util.StringUtils;
import org.staxer.util.http.helper.HttpHelper;
import org.staxer.util.http.helper.HttpMethod;
import org.staxer.util.http.helper.ReadHttpParameters;
import org.staxer.util.http.response.HttpResponseContentType;
...
public abstract class WsMessageProcessor {
    ...
    public void process(
        ...
        try {
            ...
            if (requestParametersNames.contains("wsdl") || requestParametersNames.contains("def")) {
                ...
                String def = httpHelper.getSafeRequestParameter("def");
                if (def == null) {
                ...
                if (HttpMethod.GET == httpHelper.getMethod()) {
                    requestXmlName = new XmlName(
                            httpHelper.getSafeRequestParameter("namespace"),
                            httpHelper.getSafeRequestParameter("operation", "empty")
                    );
    ...
    public WsMessage buildMessage(
        ...
        if (auth == null) {
            auth = httpHelper.getSafeRequestParameter("Authorization");
        }
