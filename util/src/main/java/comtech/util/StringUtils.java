...
public class StringUtils {
    ...
    private static final String PNR_NUMBER_RU_SYMBOLS = "БВГДКЛМНПРСТФХЦЖШ";
    private static final String PNR_NUMBER_EN_SYMBOLS = "BVGDKLMNPRSTFXCZW";

    private static final Map<Character, Character> PNR_NUMBER_TO_RU_MAP;
    private static final Map<Character, Character> PNR_NUMBER_TO_EN_MAP;
    ...
    static {
        ...
        PNR_NUMBER_TO_EN_MAP = new HashMap<Character, Character>();
        PNR_NUMBER_TO_RU_MAP = new HashMap<Character, Character>();

        int count = PNR_NUMBER_RU_SYMBOLS.length();
        for (int i = 0; i < count; ++i) {
            char ruChar = PNR_NUMBER_RU_SYMBOLS.charAt(i);
            char enChar = PNR_NUMBER_EN_SYMBOLS.charAt(i);
            PNR_NUMBER_TO_EN_MAP.put(ruChar, enChar);
            PNR_NUMBER_TO_RU_MAP.put(enChar, ruChar);
        }
    }
...
// Encode a byte array as a hex encoded string.
    ...
    public static String replaceToSirenaString(String string) {
        if (string != null) {
            return string.replaceAll("ъ", "ь").replaceAll("ё", "е").replaceAll("Ё", "Е").replaceAll("Ъ", "Ь");
        } else {
            return null;
        }
    }
    ...
    public static String pnrNumberToEn(String src) {
        return translate(src, PNR_NUMBER_TO_EN_MAP);
    }

    public static String pnrNumberToRu(String src) {
        return translate(src, PNR_NUMBER_TO_RU_MAP);
    }
